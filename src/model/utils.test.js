...
test('Boxcox power transform unit test.', () => {
  ...
  const error = tf.abs(tf.sub(boxcoxOut, tf.sub(timeseries, 1)));

  const tolerance = 0.00001;
  const condition = tf.min(tf.less(error, tolerance)).dataSync();
